  ...
  useEffect(() => {
    ...
    const image = match(logo)
      ...
      .otherwise(logo => {
        ...
        // (this app's createSvgImage throws instead of returning a Result, as lake's used to)
        return Result.fromExecution(() => createSvgImage(whiteLogo)).mapError(error =>
    ...
    const applyLogo = () => {
      ...
      const { width: logoWidth, height: logoHeight } = computeCardLogoSize(logoImage);
      const alphaMap = new THREE.Texture(logoImage);
    ...
    // The image was just given a data URI src and hasn't decoded yet. Uploading it to a
    // texture in that state yields an empty alpha map, which renders the logo plane as a
    // solid rectangle instead of masking it to the logo's shape.
    if (logoImage.complete && logoImage.naturalWidth > 0) {
      applyLogo();
    } else {
      logoImage.decode().then(applyLogo, () => {
        if (!cancelled) {
          handleSvgError.current?.("logoDecodeFailed");
        }
      });
    }
